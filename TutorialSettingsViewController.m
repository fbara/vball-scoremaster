...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSArray *coachMarks = @[
			...
			@{
                @"rect": [NSValue valueWithCGRect:(CGRect){{6.0f,143.0f},{557.0f,89.0f}}],
            ...
            @{
                @"rect": [NSValue valueWithCGRect:(CGRect){{6.0f,229.0f},{552.0f,86.0f}}],
            ...
            @{
                @"rect": [NSValue valueWithCGRect:(CGRect){{84.0f,38.0f},{233.0f,243.0f}}],
                @"caption": NSLocalizedString(@"If your device supports 3D Touch, use it on the VBall icon.", @"Shown during the app tutorial"),
...
//	}
    ...
    if (index == 6) {
        [self.tutorialImage setImage:[UIImage imageNamed:@"Home Screen 3D Edit"]];
        return;
    }
