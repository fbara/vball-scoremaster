...
- (void)viewDidLoad {
  ...
  homeSwipeGesture.direction = UISwipeGestureRecognizerDirectionLeft |
  ...
  UISwipeGestureRecognizer *visitorSwipeGesture =
      [[UISwipeGestureRecognizer alloc] initWithTarget:self
                                                action:@selector(handleSwipe:)];
  visitorSwipeGesture.direction = UISwipeGestureRecognizerDirectionLeft |
                                  UISwipeGestureRecognizerDirectionRight;
  ...
  if (![ABXPromptView hasHadInteractionForCurrentVersion]) {
    if ((([[NSUserDefaults standardUserDefaults]
      ...
      self.promptView.autoresizingMask =
...
- (UIColor *)colorHomeScoreView {
  ...
  if (theHomeData != nil) {
    colorHome =
        (UIColor *)[NSKeyedUnarchiver unarchiveObjectWithData:theHomeData];
  } else {
  ...
  if ([color isEqualToString:@"Complementary"] ||
    ...
    self.homeTeamName.textColor =
        ContrastColorOf(self.homeTeamName.backgroundColor, TRUE);
  } else {
...
- (UIColor *)colorVisitorScoreView {
  ...
  if (theVisitorData != nil) {
    colorVisitor =
        (UIColor *)[NSKeyedUnarchiver unarchiveObjectWithData:theVisitorData];
  } else {
  ...
  if ([color isEqualToString:@"Complementary"] ||
      [color isEqualToString:@"Dark"]) {
    self.visitingTeamName.backgroundColor =
        ComplementaryFlatColorOf(colorVisitor);
    self.visitingTeamName.textColor =
        ContrastColorOf(self.visitingTeamName.backgroundColor, TRUE);
...
- (void)windowBackgroundColor {
  ...
  if ([[defaults objectForKey:@"colorSettings"]
    ...
    self.navigationController.navigationBar.tintColor =
        ContrastColorOf(FlatSkyBlue, TRUE);
    ...
    self.gameNumber.textColor =
        ContrastColorOf(self.view.backgroundColor, TRUE);
    ...
    self.navigationController.navigationBar.tintColor =
        ContrastColorOf(FlatBlack, TRUE);
    self.navigationController.navigationBar.titleTextAttributes = @{
    ...
    self.navigationController.navigationBar.tintColor =
        ContrastColorOf(FlatNavyBlue, TRUE);
    self.navigationController.navigationBar.titleTextAttributes = @{
...
- (void)formatVBallButton {
  ...
  self.sendMessageImage.layer.cornerRadius =
      self.sendMessageImage.frame.size.width / 2.0f;
...
- (UIViewController *)pageViewController:
  ...
  // properties
  DefaultScoreViewController *oldViewController =
      (DefaultScoreViewController *)viewController;
